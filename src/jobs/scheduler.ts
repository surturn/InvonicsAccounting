...
export const startScheduler = () => {
  ...
  logger.info('Registered cron: 0 8 1 * * -> sendTOTReminder (Africa/Nairobi)');

  cron.schedule('0 7 * * 1', sendWeeklyCashPosition, options);
  logger.info('Registered cron: 0 7 * * 1 -> sendWeeklyCashPosition (Africa/Nairobi)');

  cron.schedule('0 9 2 * *', sendMonthlyExport, options);
  logger.info('Registered cron: 0 9 2 * * -> sendMonthlyExport (Africa/Nairobi)');

  cron.schedule('0 10 21 * *', autoLockPeriod, options);
  logger.info('Registered cron: 0 10 21 * * -> autoLockPeriod (Africa/Nairobi)');
};
